...
lm.1.2 <- lm(log(IncomePovertyRatio) ~ SameResidenceWorkplace*JWMNP, data=dat)
result1.2 <- summary(lm.1.2)
...
# 2nd Model: include secondary core vars "JWTR" and interaction
#            (time to commute and transportation mean will be related)
#sink("OLS_output2.txt")
dat <- dat[!is.na(dat$JWMNP),]
lm.4 <- lm(log(IncomePovertyRatio) ~ SameResidenceWorkplace*JWMNP+JWTR*JWMNP, data=dat)
result4 <- summary(lm.4)
#cat("\nOLS regression output with another interaction term: \n")
#result4
#sink()

# 3rd Model: include secondary core vars "MIG"
#sink("OLS_output3.txt")
dat <- dat[!is.na(dat$JWMNP),]
lm.5 <- lm(log(IncomePovertyRatio) ~ SameResidenceWorkplace*JWMNP+JWTR*JWMNP+MIG, data=dat)
result5 <- summary(lm.5)
...
# 5th Model: Now include all "society"  factors not included in 4th model
